...
**Free text** is just that. It can be included anywhere in an [abc file](#abc_file_definition "abc:standard:v2.1 ↵"), after the [file header](#file_header_definition "abc:standard:v2.1 ↵"), but must be separated from [abc tunes](#abc_tune_definition "abc:standard:v2.1 ↵"), [typeset text](#typeset_text_definition "abc:standard:v2.1 ↵") and the [file header](#file_header_definition "abc:standard:v2.1 ↵") by [empty lines](#empty_line_definition "abc:standard:v2.1 ↵"). Typically it is used for annotating the [abc file](#abc_file_definition "abc:standard:v2.1 ↵") but in principle can be any text not containing [information fields](#information_field_definition "abc:standard:v2.1 ↵").
...
*   most [stylesheet directives](#stylesheet_directive_definition "abc:standard:v2.1 ↵") are too short to require a continuation syntax, but if one is required then use the `I:<directive>` form (see `[I:instruction](#iinstruction "abc:standard:v2.1 ↵")`), in place of `%%<directive>` and continue the line as a field - see [field continuation](#field_continuation "abc:standard:v2.1 ↵").
...
*  Field name                                    :  O:origin                         
  ...
  * inline:       
  *  type        :  string      
  *  Examples and notes :  O:UK; Yorkshire; Bradford                              
*  Field name                                    :  P:parts                           
  *  file header :      
  *  tune header :  yes    
  *  tune body:  yes 
...
_Alternative usage example:_ The above could also be achieved using [inline fields](#inline_field_definition "abc:standard:v2.1 ↵"), the `[I:<directive>](#iinstruction "abc:standard:v2.1 ↵")` form instead of `%%<directive>` and a `[r:remark](#comments_and_remarks "abc:standard:v2.1 ↵")` in place of the [comment](#comment_definition "abc:standard:v2.1 ↵"), i.e.
...
Note that the `y` symbol does _not_ create [rests](#rests "abc:standard:v2.1 ↵") in the music.
...
_VOLATILE:_ See [above](#multiple_voices "abc:standard:v2.1 ↵").
...
_VOLATILE:_ See [above](#multiple_voices "abc:standard:v2.1 ↵"). In particular the following may be relaxed with the introduction of a control voice.
...
_Example:_ In the following excerpt, using abc 2.0 the lyrics would be aligned with the adjacent [music code](#music_code_definition "abc:standard:v2.1 ↵"), i.e. with `cdef`; using abc 2.1 they would be aligned at the start of the tune (or voice), i.e. with `CDEF`.
...
Alternatively, any [stylesheet directive](#stylesheet_directive_definition "abc:standard:v2.1 ↵") may be written as an `[I:instruction](#iinstruction "abc:standard:v2.1 ↵")` field although this is not recommended for usages which have not been standardised (i.e. it is not recommended for any directives described in section 11).
...
_VOLATILE:_ This section is under review as part of the general discussion about [multiple voices](#multiple_voices "abc:standard:v2.1 ↵") for abc 2.2. See also the [section 11 disclaimer](#disclaimer "abc:standard:v2.1 ↵").
...
_VOLATILE:_ See the [section 11 disclaimer](#disclaimer "abc:standard:v2.1 ↵").
...
_VOLATILE:_ This section is under active discussion. See also the [section 11 disclaimer](#disclaimer "abc:standard:v2.1 ↵").
...
_VOLATILE:_ Font directives are due to be considered in abc 2.3 - see the [section 11 disclaimer](#disclaimer "abc:standard:v2.1 ↵").
...
_VOLATILE:_ Text directives are due to be considered in abc 2.3 - see the [section 11 disclaimer](#disclaimer "abc:standard:v2.1 ↵").
...
_Recommendation for users:_ If you are using text directives for tune-specific information, consider instead using one of the [background information fields](#bdfsbackground_information "abc:standard:v2.1 ↵") together with a `%%writefields` directive (see [information directives](#information_directives "abc:standard:v2.1 ↵")) so that the information can correctly identified by databasing software.

#### 11.4.6 Information directives

_VOLATILE:_ The `%%writefields` directive and its formatting options are likely to be enhanced when markup is considered in abc 2.3. See also the [section 11 disclaimer](#disclaimer "abc:standard:v2.1 ↵").
...
_Comment:_ The `$` symbol is effectively a replacement for `!`. It is aimed at those users who want `!` as the decoration delimiter but who prefer to use [code line-breaks](#code_line-break_definition "abc:standard:v2.1 ↵") without generating corresponding [score line-breaks](#score_line-break_definition "abc:standard:v2.1 ↵").
...
_Recommendation for users:_ Given the very small uptake of the `+` symbol for decorations, "`I:decoration +`" directive is not recommended. However, it is retained for users who wish to use the `!` symbol for line-breaking in legacy [abc files](#abc_file_definition "abc:standard:v2.1 ↵").
